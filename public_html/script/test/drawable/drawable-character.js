...
describe('drawable-character.js', () => {
    const character = new DrawableCharacter({frame: new Rect(20, 20, 40, 30),
            imageSet: new Image(800, 800), subImageWidth: 40, subImageHeight: 30,
            imagesetBoundsList: [new Rect(0, 0, 800, 200), new Rect(0, 200, 800, 200),
    ...
    describe('set direction', () => {
        it('should throw a TypeError if direction is not in the Direction enum', () => {
            const typeError = () => { character.direction = 5; };
        ...
        it('should modify DrawableSprite[SUBIMAGE_BOUNDS] correctly', () => {
            ...
            bounds.should.not.eql(character.subImageBounds);
            character.direction = Direction.down;
            bounds.should.eql(character.subImageBounds);
        });
    });
});
